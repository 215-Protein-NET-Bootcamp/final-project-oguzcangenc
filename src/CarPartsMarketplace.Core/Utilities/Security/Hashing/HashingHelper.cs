...
namespace CarPartsMarketplace.Core.Utilities.Security.Hashing
{
    public static class HashingHelper
    {
        ...
        [Obsolete("Obsolete")]
        public static void MD5Hash(string mail, out string mailMd5, IOptions<TokenOptions> _tokenOptions)
        {
            MD5 md5 = new MD5CryptoServiceProvider();
            var securityKey = _tokenOptions.Value.SecurityKey;

            md5.ComputeHash(ASCIIEncoding.ASCII.GetBytes(mail + securityKey));
